...
function FeaturedProjectCarousel({ project }: FeaturedProjectCarouselProps) {
  ...
  const slides = project.gallery && project.gallery.length > 0 ? project.gallery : fallbackSlides
  ...
  return (
    ...
    >
      {activeSlide.imageSrc ? (
        <img src={activeSlide.imageSrc} alt={activeSlide.title} className="block h-full w-full object-cover" />
      ) : (
        <div
      ...
      {slides.length > 1 ? (
        <>
          <button
            ...
            className="absolute left-0 top-1/2 inline-flex h-16 w-10 -translate-y-1/2 items-center justify-center rounded-r-full border border-l-0 border-white/34 bg-white/78 pr-1 text-slate-900 shadow-[0_10px_24px_rgba(15,23,42,0.14)] backdrop-blur-sm transition-[opacity,background-color] duration-200 hover:bg-white focus-visible:bg-white focus-visible:outline-none md:h-[74px] md:w-11 md:opacity-0 md:pointer-events-none md:group-hover:opacity-100 md:group-hover:pointer-events-auto md:group-focus-within:opacity-100 md:group-focus-within:pointer-events-auto"
          ...
          <button
            ...
            className="absolute right-0 top-1/2 inline-flex h-16 w-10 -translate-y-1/2 items-center justify-center rounded-l-full border border-r-0 border-white/34 bg-white/78 pl-1 text-slate-900 shadow-[0_10px_24px_rgba(15,23,42,0.14)] backdrop-blur-sm transition-[opacity,background-color] duration-200 hover:bg-white focus-visible:bg-white focus-visible:outline-none md:h-[74px] md:w-11 md:opacity-0 md:pointer-events-none md:group-hover:opacity-100 md:group-hover:pointer-events-auto md:group-focus-within:opacity-100 md:group-focus-within:pointer-events-auto"
            data-projects-carousel-nav="true"
          >
            <svg viewBox="0 0 24 24" aria-hidden="true" className="h-4 w-4" fill="none">
              <path
            ...
            {slides.map((slide, index) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-200 ${
                  index === activeIndex ? 'w-6 bg-white' : 'w-2 bg-white/48'
                }`}
