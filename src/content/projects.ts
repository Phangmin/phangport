...
export const projectsByLanguage: Record<LanguageCode, ProjectPageProject[]> = {
  ko: [
    {
      ...
      stacks: ['Electron', 'React', 'TypeScript', 'Tailwind CSS', 'SQLite3', 'Vite'],
    ...
    },
    {
      ...
      introduction:
        '실시간 영상 상담과 차트 드로잉을 하나의 흐름으로 연결해, 상담사와 사용자가 같은 맥락에서 빠르게 의사결정을 내릴 수 있도록 설계한 프로젝트',
      features: [
        'WebRTC 기반 양방향 화상 상담 및 주식 차트 호출과 실시간 양방향 드로잉 기능을 통한 상담 용이성 및 이해도 증대',
        '전문가 가입 시 자격인증을 통한 신뢰성 확보 및 결제 시스템 구축',
        '투자 지식in 활용을 통한 컨설턴트와 상담자 간의 연결 창구 제공 및 자연스러운 영업활동 UX 제공',
      ],
      troubleshooting:[
        '[문제] 전문가 페이지 내 프로필 이미지 로딩 실패 및 데이터 불일치 현상 발생',
        '[원인] 프론트엔드 콘솔 로그 분석 후 이상 없음을 확인, 벡엔드 로직을 직접 디버깅한 결과 이미지 업로드 시 AWS S3 저장 경로와 DB 참조 경로가 상이하게 설계된 것을 발견.(사용자 프로필과 전문가 이미지 저장 로직의 파라미터 불일치)',
        '[해결] 백엔드 팀원과 소통하여 코드상 경로 변수를 통일하고, 누락된 도메인 네임 규칙을 수정하여 데이터 정합성을 확보',
      ],
      results: [
        '기술 협업 효율성 증대: 직접적인 백엔드 코드 디버깅을 통해 이슈 해결 시간을 단축(3시간)하고, Figma 독학을 통해 팀 내 소통 비용을 획기적으로 절감',
        '시스템 안정성 강화: 결제 및 가입 프로세스와 직결된 전문가 신뢰도(프로필 이미지) 이슈를 해결하여 서비스 완성도 제고',
        'UX 통합: 실시간 상담 중 맥락 전환 비용을 줄이는 통합 인터페이스 구축 및 UI/UX 표준 가이드 수립'
      ],
      stacks: ['React', 'TypeScript', 'OpenVidu','Vite', 'Tailwind CSS', 'MySQL', 'Zustand', 'Chart.js', 'Konva.js', 'KRX API', 'KIS API', 'Figma'],
      contribution: '프로젝트 전반의 프론트엔드 및 UI/UX 총괄, 전문가 페이지 담당',
      contributionRate: 25,
      type: 'team',
      teamSize: 6,
      hasAward: true,
    ...
    },
    {
      ...
      features: [
        '사용자 선호 및 독서 경험 기반 AI 기반 도서 추천 Top5',
        'AI와 함께 써내는 새로운 나만의 이야기, 열린 결말 제공',
        '작성된 독후감을 토대로 감정을 분석해 특별한 표지를 생성하는 AI 커버 서비스',
      ],
      troubleshooting: [
        '[문제] FE와 BE 간 JWT 인증 구현 중, 새로고침 시 인증 상태가 초기화되는 UX 저하 발생',
        '[원인] JWT Access Token이 클라이언트 상태 값(State)으로만 관리하여 페이지 리로드 시 메모리 휘발로 인한 데이터 유실 확인',
        '[해결 - 토큰저장] 보안을 고려하여 Refresh Token은 HttpOnly Cookie에, Access Token은 메모리에 저장하는 전략 채택',
        '[해결 - 인증흐름] Access Token이 메모리에만 존재하므로, 페이지 새로고침 시 백엔드에 Refresh Token을 이용한 Access Token 재발급 요청을 보내도록 인증 흐름 재구성',
        '[해결 - 구현] useEffect를 활용해 앱 초기 로딩 시 Silent Refresh 로직으로 구현하여 새로고침 후에도 로그인 상태가 유지되도록 개선',
      ],
      results: [
        '사용자 경험 최적화: AI 기반 독서 기록 경험을 짧은 입력 흐름으로 재구성하여 편의성 증대',
        '기술적 안정성 확보: Silent Refresh 로직 구현을 통해 새로고침 시 로그인 유지율 100% 달성 및 보안 수준 강화',
        '성과: 프로젝트 완성도와 발표력을 인정받아 관통 프로젝트 우수상 수상',
      ],
      stacks: ['Django', 'Vue.js', 'Bootstrap CSS', 'Vite', 'Pinia', 'SQLite3', 'Figma'],
    ...
    },
    {
      ...
      stacks: ['TypeScript', 'Node.js', 'Visual Studio Code', 'WebStorm', 'React', 'React Router', 'MQTT', 'Axios', 'LottieFiles', 'Vite', 'Tailwind CSS', 'Three.js', 'PWA'],
  ...
  en: [
    ...
    },
    {
      ...
      summary: 'A real-time consulting platform designed around a bidirectional stock-consulting experience built with WebRTC.',
      introduction:
        'This project connected live video consulting and stock-chart drawing into a single flow so that consultants and users could make decisions quickly within the same context.',
      features: [
        'A WebRTC-based consulting experience that combined two-way video calls, live stock-chart views, and real-time collaborative drawing',
        'A trusted expert onboarding flow with qualification verification and an integrated payment system',
        'A connection channel between consultants and users powered by investment insights, with a more natural UX for sales activity',
      ],
      troubleshooting: [
        '[Problem] Profile images on the expert page failed to load, and related data became inconsistent.',
        '[Cause] After confirming there was no issue in the frontend console logs, I debugged the backend logic directly and found that the AWS S3 upload path and the DB reference path had been designed differently, causing a mismatch in the parameters used for user-profile and expert-image storage.',
        '[Solution] I coordinated with the backend teammate to unify the path variables in code and corrected the missing domain-naming rules to restore data consistency.',
      ],
      results: [
        'Improved technical collaboration efficiency by debugging backend code directly, reducing issue-resolution time by about three hours, and lowering communication overhead through self-directed Figma work.',
        'Strengthened system stability by resolving a profile-image issue tied directly to expert trust, signup, and payment flows, which raised the overall completeness of the service.',
        'Established an integrated interface and UI/UX guideline that reduced context-switching costs during real-time consultation.',
      ],
      stacks: ['React', 'TypeScript', 'OpenVidu', 'Vite', 'Tailwind CSS', 'MySQL', 'Zustand', 'Chart.js', 'Konva.js', 'KRX API', 'KIS API', 'Figma'],
      contribution: 'Led the overall front-end and UI/UX work for the project, with ownership of the expert page.',
      contributionRate: 25,
      type: 'team',
      teamSize: 6,
      hasAward: true,
    ...
    },
    {
      ...
      summary: 'A reading platform that uses AI to help users record and share their reading experiences.',
      introduction:
        'A reading-sharing service designed so users can leave their own reading experiences without pressure and naturally connect with other people\'s records.',
      features: [
        'Top 5 AI book recommendations based on user preferences and reading experiences',
        'An open-ending story feature that lets users co-write new personal narratives with AI',
        'An AI cover service that analyzes emotions in each review and generates a distinctive cover image',
      ],
      troubleshooting: [
        '[Problem] JWT authentication between the frontend and backend reset the signed-in state on page refresh, which degraded the user experience.',
        '[Cause] The JWT access token was stored only in client-side state, so a page reload wiped it from memory.',
        '[Solution - token storage] Adopted a split-token strategy: store the refresh token in an HttpOnly cookie and keep the access token in memory.',
        '[Solution - auth flow] Reworked the auth flow so that on page refresh the client requests a new access token from the backend using the refresh token.',
        '[Solution - implementation] Implemented a silent refresh flow with useEffect during app initialization so the login state persists after refresh.',
      ],
      results: [
        'Optimized the user experience by restructuring the AI-powered reading-record flow into a shorter and lighter input experience.',
        'Improved technical stability and security by implementing silent refresh, achieving persistent login after refresh.',
        'Won the excellence award in recognition of the project\'s completeness and presentation.',
      ],
      stacks: ['Django', 'Vue.js', 'Bootstrap CSS', 'Vite', 'Pinia', 'SQLite3', 'Figma'],
